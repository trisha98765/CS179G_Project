...
avg_flight_prices.show()
 Graph 1: # of Workers vs. Execution Time in seconds with a fixed dataset size. Analysis: As the number of workers increases, the execution time increases.
...
Dacia:
- Wrote code to read a subset of data from the original Kaggle file, then created smaller
subsets for the graphs that operated on different subsets of the data.
- Wrote the project report
Isiah:
- Created Project MYSQL database, wrote code, as well as SQL queries, to create tables
based on the PySpark queries and store the returned PySpark query values within these
tables.
- Wrote the project report
Trisha:
- Wrote PySpark queries for data analysis, then created graphs to show execution time for
different number of workers and different file sizes on a PySpark query.
- Wrote the project report
 
 Part 3
Part 2 Demo Feedback
Allow for comparison of flight prices with direct flights and various numbers of stops.
Part 2 Demo Feedback Comments
The output on our website will show all the information about direct flights and flights with various numbers of stops based on the airports the user has selected. This occurs when the user inputs a starting and destination airport on the bottom of the screen. Various pieces of information about the selected airports appear on the screen.
Requirements
...
● When selecting two airports, the average price of direct flights and flights with layovers
would be displayed on the screen from queries that were written in PySpark with the data stored in MySQL.
Design
...
Part 3 Contributions Dacia:
   
 - Worked on presentation slides. Worked on setting up the interactive map of the US and set the points of each airport in our dataset. Added this map onto the website.
Isiah:
- Worked on presentation slides. Worked on setting up our webpage. Worked on displaying
our stored queries on the map. Trisha:
- Worked on presentation slides. Worked on requirements, design, implementation, front-end technology, MySQL Database Implementation, submitting a query, and screenshots section of the final report.
